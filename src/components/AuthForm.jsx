...
const AuthForm = ({ type, onSubmit, isLoading, serverError }) => {
  const isSignup = type === "signup";
  const [showPassword, setShowPassword] = useState(false);
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      {/* Server error — wrong password / user not found */}
      {serverError && (
      ...
      <p className="text-center text-sm text-muted-foreground">
        {isSignup ? (
          ...
          <>
            ...
            <Link to="/signup" className="text-primary hover:underline font-medium">Create new account</Link>
          </>
        )}
      </p>
    </form>
