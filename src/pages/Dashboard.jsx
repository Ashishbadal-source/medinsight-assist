...
const dummyData = {
  report: { type: "Complete Blood Count (CBC)", date: "December 18, 2024", confidence: 87 },
  findings: [
    { testName: "Hemoglobin", value: "14.2", unit: "g/dL", normalRange: "12.0-16.0", status: "Normal" },
    { testName: "White Blood Cells", value: "11.5", unit: "K/uL", normalRange: "4.5-11.0", status: "High" },
    { testName: "Platelet Count", value: "245", unit: "K/uL", normalRange: "150-400", status: "Normal" },
  ...
  conditions: [
    { name: "Possible Hypertension", probability: 72, reason: "Elevated blood pressure readings (145/92 mmHg) and borderline cholesterol levels suggest cardiovascular risk." },
    { name: "Mild Infection or Inflammation", probability: 58, reason: "White blood cell count slightly elevated (11.5 K/uL) which may indicate an ongoing immune response." },
    { name: "Hyperlipidemia Risk", probability: 45, reason: "Borderline cholesterol levels (215 mg/dL) warrant lifestyle modifications and monitoring." },
  ],
  recommendations: [
    { type: "specialist", text: "Consult a cardiologist for blood pressure evaluation", priority: "high" },
    { type: "retest", text: "Repeat lipid panel in 4-6 weeks after dietary changes", priority: "normal" },
  ...
  imaging: {
    imageUrl: null, hasHeatmap: true,
    analysis: "Chest X-ray shows clear lung fields with no significant abnormalities. Heart size within normal limits.",
    type: "X-ray", region: "Chest PA View", qualityScore: "Good", highlightedRegions: [],
  },
};
        ...
        <div className="max-w-7xl mx-auto">
          <div className="mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              ...
              <h1 className="text-2xl md:text-3xl font-bold text-slate-900">Analysis Dashboard</h1>
              <p className="text-slate-500 mt-1 text-sm">Review the AI-assisted analysis of your medical report</p>
            </div>
            <button
              onClick={handleExportPDF}
              disabled={isExporting}
              className="inline-flex items-center justify-center gap-2 bg-primary text-white px-5 py-2.5 rounded-xl font-medium hover:bg-primary/90 transition-all shadow-md shadow-primary/20 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {isExporting ? <><Loader2 className="h-4 w-4 animate-spin" />Exporting...</> : <><Download className="h-4 w-4" />Export PDF</>}
            </button>
          </div>

          <div ref={reportRef}>
            <div className="hidden print:block mb-6 pb-4 border-b border-border">
              <h1 className="text-xl font-bold">MedInsight AI - Medical Report Analysis</h1>
              <p className="text-sm text-muted-foreground">Generated on {new Date().toLocaleDateString()}</p>
            </div>
            ...
            <div className="mt-6"><DisclaimerBox variant="warning" /></div>
          </div>
        </div>
      </main>
      <Footer />
