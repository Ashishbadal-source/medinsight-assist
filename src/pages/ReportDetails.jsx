...
const ReportDetails = () => {
  ...
  // Filter detected conditions above 60%
  const detectedConditions = analysis?.detected_conditions?.filter(c => c.probability >= 60) || [];
            ...
            {report.report_type === "Blood Test" && analysis ? (
              ...
              <>
                ...
                <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6">
                  ...
                  <div className={`flex items-center justify-between rounded-xl p-5 border ${severityColor(detectedConditions.length > 2 ? "high" : detectedConditions.length > 0 ? "mild" : "normal")}`}>
                    <div>
                      <p className="text-lg font-bold">
                        {detectedConditions.length === 0
                          ? "No significant findings detected."
                          : detectedConditions.length <= 2
                          ? `${detectedConditions.length} finding(s) detected — mild concern.`
                          : `${detectedConditions.length} finding(s) detected — please consult a doctor.`}
                      </p>
                      <p className="text-sm mt-1 opacity-70">{analysis.total_conditions} conditions screened</p>
                    </div>
                    <div className="text-right">
                      <p className="text-3xl font-bold">{detectedConditions.length}</p>
                      <p className="text-xs opacity-70">Detected</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6">
                  ...
                  <div className="space-y-2">
                    {analysis.findings?.map((c, i) => (
                      <div key={i} className={`flex items-center justify-between p-3 rounded-xl text-sm ${c.probability >= 60 ? "bg-red-50 border border-red-100" : "bg-slate-50"}`}>
                        <span className={`font-medium ${c.probability >= 60 ? "text-red-800" : "text-slate-600"}`}>{c.condition}</span>
                        <div className="flex items-center gap-2">
                          <div className="w-24 h-1.5 bg-slate-200 rounded-full overflow-hidden">
                            <div className={`h-full rounded-full ${xrayBarColor(c.probability)}`} style={{ width: `${c.probability}%` }} />
                          </div>
                          <span className={`text-xs font-semibold w-10 text-right ${c.probability >= 60 ? "text-red-600" : "text-slate-400"}`}>{c.probability}%</span>
                        </div>
                      </div>
                    ))}
                  </div>
