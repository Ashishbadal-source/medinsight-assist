# import cv2
# import numpy as np
# from llm.llm_ecg import analyze_ecg as llm_analyze
# import os
...
#     # 3️⃣ Prepare ECG report for LLM
#     ecg_report = f"""
# Heart Rate: 105 bpm
# Rhythm: Irregular
# ST Elevation: Present
# Diagnosis: {'Abnormal' if ml['prediction']==1 else 'Normal'}
# Confidence: {ml['confidence']}
# """

#     # 4️⃣ LLM explanation
#     llm_output = llm_analyze(ecg_report)
...
def ml_predict(ecg):
    ...
    return {
        "prediction": 1,
        "confidence": 0.82,
        "heart_rate": "102 bpm",      # ← real model se aayega baad mein
        "rhythm": "Irregular",         # ← real model se aayega baad mein
        "st_elevation": "Present"      # ← real model se aayega baad mein
    }
...
def analyze_ecg(image_path: str):
    ...
    # 3️⃣ Prepare ECG report string for LLM
    diagnosis_label = 'Abnormal' if ml['prediction'] == 1 else 'Normal'
    confidence_pct = round(ml['confidence'] * 100, 1)

    ecg_report = f"""
Heart Rate: {ml.get('heart_rate', 'N/A')}
Rhythm: {ml.get('rhythm', 'N/A')}
ST Elevation: {ml.get('st_elevation', 'N/A')}
